...
TABLE filter(TABLE table, char columName[], char *value, OPERATOR oper)
{
    ...
    while (j < getNumLinTotal(table))
    {
        ...
        if (compare(variaveis[j][col], value, oper) == 0)
            flag = 1;
...
int interpretador(SGR sgr)
{
    ...
    while (linha)
    {
        ...
        if (isAssignment(linha))
            strcpy(funcao, info[2]);
        else
            strcpy(funcao, info[0]);
        ...
        {
            if (!strcmp(info[1], "NULL"))
                info[1] = NULL;
            if (!strcmp(info[2], "NULL"))
                info[2] = NULL;
            if (!strcmp(info[3], "NULL"))
                info[3] = NULL;
            free_sgr(sgr);
            flagQuery9 = 1;
        ...
        else if ((!(strcmp("show", funcao))) && (length == 3))
        {
            ...
            if (posicao != -1)
                show(vars[posicao].table);
            else
                printf("A TABLE nao existe\n");
        ...
        else if ((!(strcmp("toCSV", funcao))) && (length == 5))
        {
            int posicao = verificaVar(vars, i, info[1]);
            if (posicao != -1)
                toCSV(vars[posicao].table, info[2][0], info[3]);
            else
                printf("A TABLE nao existe\n");
        }
        ...
        else if (!strcmp("quit", funcao))
        {
            free_sgr(sgr);
            return EXIT_CODE;
        }
        else if (!strcmp("q", funcao))
        {
            free_sgr(sgr);
            return EXIT_CODE;
        }
